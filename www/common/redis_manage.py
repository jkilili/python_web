...
class RedisManage(object):
    ...
    def get_str(self,key):
            Log.info('redis string get str %s'%(key))     
            try:
                return self._redis.get(key) if self._redis.exists(key) else ''
            except Exception as e:
                Log.error('redis string get str %s error, reason:%s'%(key, e))

    def  exists_str(self,key):
        Log.info('redis string exists str %s:%s'%(key))     
        try:
            return True if self._redis.exists(key) else False
        except Exception as e:
            Log.error('redis string exists str %s error, reason:%s'%(key, e))

     # 加list头部元素
    def set_lpush(self,key, value):
            Log.info('redis list set lpush %s:%s'%(key,value))
            try:
                value = json.dumps(value, ensure_ascii=False, default=None, cls = jsonConverter).encode('utf-8')
                self._redis.lpush(key,  value)
            except Exception as e:
                Log.error('redis list set lpush %s:%s error, reason:%s'%(key,value, e))
    ...
    def remove_lpop(self,key):
            Log.info('redis list set lpop %s'%(key))
            try:
                self._redis.lpop(key)
            except Exception as e:
                Log.error('redis list set lpop %s error, reason:%s'%(key, e))


    # 删除list最后一个元素            
    def remove_rpop(self, key):
            Log.info('redis list get lrange value:%s'%(key))       
            try:
                return self._redis.rpop(key)
            except Exception as e:
                Log.error('redis list get lrange value:%s error, reason:%s'%(key,e))

    #返回集合key所有元素
    def get_zset(self, key):
            Log.info('redis zset get smembers value:%s'%(key))     
            try:
                return self._redis.smembers(key, start, end)
            except Exception as e:
                Log.error('redis zset get smembers value:%s error, reason:%s'%(key,e))

    def set_zset(self, key, value):
            Log.info('redis zset get sadd %s:%s'%(key))     
            try:
                value = json.dumps(value, ensure_ascii=False, default=None, cls = jsonConverter).encode('utf-8')
                return self._redis.sadd(key, value)
            except Exception as e:
                Log.error('redis zset get sadd %s:%s error, reason:%s'%(key,value, e))

    #返回集合key删除数据个数
    def remove_zset(self, key,):
            Log.info('redis zset get srem  %s'%(key))     
            try:
                return self._redis.srem(key)
            except Exception as e:
                Log.error('redis zset get srem %s error, reason:%s'%(key, e))

                
    #添加hash key
    def set_hash(self, key, id, value):
            Log.info('redis hash get hset %s:%s:%'%(key,id,value))     
            try:
                value = json.dumps(value, ensure_ascii=False, default=None, cls = jsonConverter).encode('utf-8')
                return self._redis.hset(key, id, value)
            except Exception as e:
                Log.error('redis hash get hset %s:%s:%s error, reason:%s'%(key,id,value, e))
    
    def get_hash(self, key, id):
            Log.info('redis hash get hget %s:%s'%(key, id))     
            try:
                return self._redis.hget(key, id)
            except Exception as e:
                Log.error('redis hash get hget %s:%s error, reason:%s'%(key,id, e))

    def set_hmset(self, key, **h):
            Log.info('redis hash get hmset %s:%s'%(key, h))     
            try:
                h = json.dumps(h, ensure_ascii=False, default=None, cls = jsonConverter).encode('utf-8')
                return self._redis.hmset(key, h)
            except Exception as e:
                Log.error('redis hash get hmset %s:%s error, reason:%s'%(key,h, e))

    def set_hexists(self, key, id):
            Log.info('redis hash get hexists %s:%s'%(key))     
            try:
                return self._redis.hexists(key, id)
            except Exception as e:
                Log.error('redis hash get hexists %s:%s error, reason:%s'%(key,id, e))
    #
    def remove_hash(self, key, id):
            Log.info('redis hash get hdel %s:%s'%(key, id))     
            try:
                return self._redis.hdel(key, id)
            except Exception as e:
                Log.error('redis hash get hdel %s:%s error, reason:%s'%(key,id, e))